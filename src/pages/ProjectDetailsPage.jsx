  ...
  return (
    <Container className="project-details-page">
      ...
      <Row>
        {project.images.map((image, index) => (
          <Col key={index} md={4} className="mb-3">
            <Image src={image} alt={`Project ${index + 1}`} fluid className="project-image" />
          </Col>
        ))}
      </Row>
      <div className="project-info">
        ...
        <p>{project.scope}</p>
        <h4>Description</h4>
        <p dangerouslySetInnerHTML={{ __html: formatTextWithBold(project.description) }} />
        <h4>Technologies</h4>
        <p>{project.technologiesDetails}</p>
      </div>
    </Container>
  );
};
